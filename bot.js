...
const LOCK_IN_PROFIT_DOLLARS = 1;    // real-signal trades: lock in once profit reaches this
const WEAK_LOCK_IN_PROFIT_DOLLARS = 0.5; // weak-signal trades: lock in sooner, smaller target to begin with
...
async function main() {
  ...
  // ---------- Step 1: if we were tracking an open contract, check whether it closed, or lock in profit early ----------
  if (state.openContractId) {
    ...
    if (c && c.is_sold) {
      ...
      log(`Previous trade (contract ${state.openContractId}) still open. Current profit: $${currentProfit.toFixed(2)}. Held for ${Math.floor(secondsOpen / 60)} min.`);

      if (currentProfit >= (state.openContractIsWeak ? WEAK_LOCK_IN_PROFIT_DOLLARS : LOCK_IN_PROFIT_DOLLARS) && heldLongEnough) {
        const threshold = state.openContractIsWeak ? WEAK_LOCK_IN_PROFIT_DOLLARS : LOCK_IN_PROFIT_DOLLARS;
        log(`Profit ($${currentProfit.toFixed(2)}) has reached the lock-in threshold ($${threshold}) - selling now instead of risking it for the full target.`);
        ...
        if (sellResp.sell) {
          ...
          log(`LOCKED IN: closed early with profit $${currentProfit.toFixed(2)}. Virtual balance now $${state.virtualBalance.toFixed(2)}`);
          state.openContractId = null;
        } else {
  ...
  if (openContracts.length > 0) {
    ...
    async function placeTrade(inst, candles, isWeak) {
      ...
      if (buyResp.buy) {
        ...
        state.openContractIsWeak = isWeak; // remembered so we know which lock-in threshold to use while it's open
      } else {
