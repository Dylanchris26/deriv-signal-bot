...
async function main() {
  ...
  if (openContracts.length > 0) {
    ...
    for (const inst of INSTRUMENTS) {
      ...
      if (spike && volOk) {
        await placeTrade(inst, candles);
        tradedThisRun = true;
      } else if (!bestFallback && isFallbackSpike(candles, inst.direction) && volOk) {
        bestFallback = inst; // remember first eligible fallback candidate, keep scanning for a REAL signal first
      }
    }

    // If nothing real fired anywhere, and it's late, and we haven't traded today - use the fallback
    if (!tradedThisRun && bestFallback && state.tradesToday === 0 && utcHour >= FORCE_TRADE_UTC_HOUR) {
      log(`No real signal on any instrument today, past ${FORCE_TRADE_UTC_HOUR}:00 UTC - using fallback on ${bestFallback.symbol}`);
    ...
    if (!tradedThisRun) log('No valid signal on any instrument this run.');

    async function placeTrade(inst, candles, usingFallback = false) {
      ...
      log(`${usingFallback ? 'FALLBACK (low-conviction) ' : ''}Opening ${contractType} on ${inst.symbol}. Stake $${stake.toFixed(2)} (from virtual balance $${state.virtualBalance.toFixed(2)}) | target $${takeProfitDollar.toFixed(2)} | max loss $${stopLossDollar.toFixed(2)}`);
